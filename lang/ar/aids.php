<?php

return [
    'index_title' => 'الإعانات',
    'index_subtitle' => 'عرض وإدارة جميع الإعانات',
    'create_title' => 'إنشاء إعانة جديدة',
    'create_subtitle' => 'أنشئ إعانة جديدة لمستفيد',
    'edit_title' => 'تعديل الإعانة',
    'edit_subtitle' => 'عدّل تفاصيل الإعانة',
    'create_button' => 'إعانة جديدة',

    'field_title' => 'العنوان',
    'field_title_hint' => 'عنوان اختياري لهذه الإعانة (مثل: «سداد إيجار — يوليو»). يظهر في رأس الإعانة والقوائم.',
    'field_title_placeholder' => 'عنوان اختياري للإعانة',
    'field_beneficiary' => 'المستفيد',
    'field_beneficiaries' => 'المستفيدون',
    'field_program' => 'البرنامج',
    'field_type' => 'نوع الإعانة',
    'field_amount' => 'المبلغ',
    'field_purpose' => 'الغرض',
    'field_items' => 'العناصر',
    'field_item_name' => 'اسم العنصر',
    'field_item_quantity' => 'الكمية',
    'field_item_estimated_value' => 'القيمة المقدرة',
    'field_item_description' => 'الوصف',
    'field_notes' => 'ملاحظات',
    'field_reference' => 'الرقم المرجعي',
    'field_status' => 'الحالة',
    'field_current_stage' => 'المرحلة الحالية',
    'field_created_at' => 'تاريخ الإنشاء',
    'field_created_by' => 'أنشأه',
    'field_decision_note' => 'ملاحظة القرار',
    'decision_note_hint' => 'أضف ملاحظة توضح السبب (مطلوب للرفض والإرجاع)',

    'select_placeholder' => 'اختر من القائمة',
    'search_placeholder' => 'ابحث بالمرجع أو اسم المستفيد...',
    'beneficiary_search_placeholder' => 'ابحث بالاسم أو رقم الهوية...',
    'picker_placeholder' => 'ابحث واختر المستفيدين',
    'no_results' => 'لا يوجد مستفيدون مطابقون للبحث',
    'select_all' => 'تحديد الكل',
    'clear_selection' => 'إلغاء تحديد الكل',
    'select_all_capped' => 'تم تحديد أول :count مستفيد فقط ضمن نتائج البحث الحالية (الحد الأقصى للتحديد الجماعي دفعة واحدة). ضيّق نطاق البحث لتحديد الباقين.',
    'selected_count' => '{0} لم يُختر أي مستفيد بعد|{1} تم اختيار مستفيد واحد|[2,*] تم اختيار :count مستفيدين',
    'bulk_create_hint' => 'سيتم إنشاء إعانة مستقلة لكل مستفيد من المستفيدين الـ :count المختارين، بنفس البيانات المُدخلة أعلاه.',
    'category_merge_hint' => 'اختر تصنيفًا لإضافة جميع المستفيدين المؤهَّلين ضمنه إلى التحديد (يُستبعَد الموقوف والمعطّل والمرفوض). يمكنك بعدها الإضافة أو الحذف يدويًا.',
    'override_amounts_title' => 'مبالغ مخصصة لكل مستفيد',
    'override_amounts_hint' => 'اترك الحقل فارغًا لاستخدام المبلغ أعلاه، أو حدّد مبلغًا مختلفًا لمستفيد بعينه.',
    'selected_list_hint' => 'راجِع المستفيدين المطابقين أدناه واحذف من يجب استبعاده.',

    'type_cash' => 'إعانة نقدية',
    'type_cash_hint' => 'دفع مبلغ محدد بالريال السعودي',
    'type_in_kind' => 'إعانة عينية',
    'type_in_kind_hint' => 'توفير سلع أو خدمات محددة',

    'currency_sar' => 'ر.س',

    'add_item' => 'إضافة عنصر',
    'no_items_yet' => 'لم تُضف عناصر بعد',

    'save_draft' => 'حفظ كمسودة',
    'save_and_submit' => 'حفظ وتقديم للموافقة',
    'submit_button' => 'تقديم للموافقة',

    'confirm_submit' => 'هل أنت متأكد من رغبتك في تقديم هذه الإعانة للموافقة؟',

    'wizard' => [
        'step_beneficiaries' => 'المستفيدون',
        'step_details' => 'تفاصيل الإعانة',
        'step_schedule' => 'الدورية والمستندات',
        'step_review' => 'المراجعة',
        'next' => 'التالي',
        'back' => 'السابق',
        'review_title' => 'مراجعة قبل الحفظ',
        'review_subtitle' => 'راجِع ملخص الإعانة، ثم احفظها كمسودة أو قدّمها للموافقة.',
        'recurring_summary' => ':frequency، تبدأ :date',
        'not_recurring' => 'غير متكررة',
    ],

    'submit_confirm' => [
        'title' => 'حفظ وتقديم للموافقة',
        'subtitle' => 'راجِع التفاصيل التالية قبل الحفظ والإرسال إلى سير الموافقات.',
        'bulk_note' => 'سيتم إنشاء وتقديم :count إعانة مستقلة، واحدة لكل مستفيد مختار.',
        'items_count' => '{1} صنف واحد|[2,*] :count أصناف',
        'recurring_note' => 'سيتم إرفاق جدولة دورية: :frequency، تبدأ :date.',
        'none' => '—',
    ],

    'recurring_series' => [
        'badge' => 'ضمن سلسلة إعانات دورية',
        'title' => 'الإعانة الدورية',
        'origin' => 'هذه الإعانة تُنشئ سلسلة إعانات دورية.',
        'instance' => 'أُنشئت هذه الإعانة من خطة دورية.',
        'view_plan' => 'عرض الخطة',
        'cycle' => 'الدفعة :n',
    ],

    'submit_modal' => [
        'title' => 'تقديم الإعانة للموافقة',
        'subtitle' => 'راجِع الملخص التالي قبل إرسال الإعانة إلى سير الموافقات.',
        'notify_title' => 'ما الذي سيحدث عند التقديم',
        'notify_stage' => 'ستنتقل الإعانة إلى مرحلة «:stage» ليراجعها :role.',
        'notify_recipients' => '{1} سيصل إشعار إلى مراجِع واحد نشط.|[2,*] سيصل إشعار إلى :count مراجعين نشطين.',
        'notify_none' => 'لا يوجد مستخدم نشط بهذا الدور حاليًا، لن يصل الإشعار لأحد حتى يُسنَد الدور.',
        'channel_bell' => 'إشعار داخل النظام',
        'channel_email' => 'بريد إلكتروني',
        'no_flow' => 'لا يوجد سير موافقات نشط لهذه الإعانة، يتعذّر تقديمها حتى تُهيّأ المراحل.',
    ],

    'decision_modal' => [
        'title' => 'تأكيد القرار: :action',
        'subtitle' => 'راجِع أثر هذا القرار قبل تأكيده.',
        'effect_title' => 'أثر هذا القرار',
        'approve_next' => 'ستنتقل الإعانة إلى مرحلة «:stage» ليراجعها :role.',
        'approve_final' => 'ستُعتمد الإعانة نهائيًا، وسيصل إشعار للمستفيد عبر الرسائل النصية/واتساب.',
        'reject_effect' => 'سترفض الإعانة وتنتقل إلى الحالة «مرفوضة» نهائيًا، ولن يُرسل إشعار.',
        'return_effect' => 'ستُعاد الإعانة إلى «مسودة» ليعدّلها منشئها ويعيد تقديمها، ولن يُرسل إشعار.',
    ],

    'cancel_modal' => [
        'title' => 'إلغاء الإعانة',
        'subtitle' => 'سيتم إلغاء هذه الإعانة ولا يمكن التراجع عن ذلك. لن يُرسل أي إشعار.',
        'confirm_button' => 'تأكيد الإلغاء',
    ],

    'confirm_cancel' => 'هل أنت متأكد من رغبتك في إلغاء هذه الإعانة؟',
    'confirm_delete' => 'هل أنت متأكد من رغبتك في حذف هذه الإعانة؟',
    'confirm_decision' => 'هل أنت متأكد من هذا القرار؟',

    'no_actions_available' => 'لا توجد إجراءات متاحة حاليًا',

    'returned_notice_title' => 'تم إرجاع هذه الإعانة للتعديل',

    'details_title' => 'التفاصيل',
    'progress_title' => 'التقدم',
    'decisions_title' => 'القرارات',
    'timeline_title' => 'السجل الزمني',
    'actions_title' => 'الإجراءات',

    'timeline' => [
        'created' => 'إنشاء الإعانة',
        'submitted' => 'تقديم الإعانة للموافقة',
        'disbursement_started' => 'بدء الصرف — :method',
        'disbursement_delivered' => 'تسليم الإعانة',
        'disbursement_confirmed' => 'تدقيق التسليم (تحقق ثانٍ)',
        'confirmation_sent' => 'إرسال رابط تأكيد الاستلام',
        'confirmation_confirmed' => 'تأكيد الاستلام من المستفيد',
    ],

    'receipt' => [
        'button' => 'طباعة السند',
        'title' => 'سند صرف إعانة',
        'field_reference' => 'الرقم المرجعي',
        'field_date' => 'تاريخ الإصدار',
        'field_beneficiary' => 'المستفيد',
        'field_national_id' => 'رقم الهوية',
        'field_program' => 'البرنامج',
        'field_type' => 'نوع الإعانة',
        'field_amount' => 'المبلغ',
        'field_status' => 'الحالة',
        'items_title' => 'الأصناف',
        'no_title' => 'بدون عنوان',
        'signature_recipient' => 'توقيع المستلم',
        'signature_official' => 'توقيع المسؤول المعتمد',
        'footer_note' => 'هذا المستند صادر إلكترونيًا من نظام إدارة العطاءات — جمعية الموسى الخيرية',
    ],

    'items_total' => 'المجموع',
    'items_count' => '{0} بدون عناصر|{1} عنصر واحد|[2,*] :count عناصر',

    'filter_status' => 'التصفية حسب الحالة',
    'filter_program' => 'التصفية حسب البرنامج',
    'filter_type' => 'التصفية حسب النوع',
    'filter_receipt' => 'حالة الاستلام',

    'results_count' => '{0} لا نتائج|{1} نتيجة واحدة|[2,*] :count نتيجة',

    'empty_title' => 'لا توجد إعانات',
    'empty_description' => 'ابدأ بإنشاء إعانة جديدة للمستفيدين',

    'status' => [
        'draft' => 'مسودة',
        'submitted' => 'مُقدَّمة',
        'under_review' => 'قيد المراجعة',
        'approved' => 'معتمدة',
        'in_disbursement' => 'قيد الصرف',
        'delivered' => 'مُسلَّمة',
        'confirmed' => 'مؤكَّدة الاستلام',
        'rejected' => 'مرفوضة',
        'cancelled' => 'ملغاة',
    ],

    'type' => [
        'cash' => 'نقدية',
        'in_kind' => 'عينية',
    ],

    'program_type' => [
        'cash' => 'نقدية فقط',
        'in_kind' => 'عينية فقط',
        'both' => 'نقدية وعينية',
    ],

    'documents' => [
        'title' => 'المستندات الداعمة',
        'hint' => 'أرفق مستندات (PDF أو JPG أو PNG، حتى 5 ميجابايت لكل ملف).',
        'uploading' => 'جارٍ الرفع...',
        'pending_count' => '{1} ملف واحد جاهز للرفع|[2,*] :count ملفات جاهزة للرفع',
    ],

    'recurrence' => [
        'card_title' => 'الإعانة الدورية',
        'card_subtitle' => 'جدولة إنشاء هذه الإعانة تلقائيًا على فترات منتظمة.',
        'enable' => 'جعل هذه الإعانة متكررة',
        'enable_hint' => 'إنشاء إعانة جديدة تلقائيًا وفق جدول زمني.',
        'frequency_label' => 'التكرار',
        'interval_months' => 'عدد الأشهر بين كل تكرار',
        'starts_on' => 'تاريخ بدء الجدولة',
        'starts_on_hint' => 'متى تصبح الجدولة فعّالة — لا تُنشأ أي إعانة قبل هذا التاريخ.',
        'due_on' => 'تاريخ أول استحقاق',
        'due_on_hint' => 'تاريخ الاستحقاق الأول للإعانة. اتركه فارغًا لاستخدام تاريخ البدء.',
        'ends_on' => 'تاريخ الانتهاء (اختياري)',
        'ends_on_hint' => 'اتركه فارغًا للتكرار بلا نهاية.',
        'lead_days' => 'أيام التهيئة المسبقة',
        'lead_days_hint' => 'عدد الأيام قبل موعد الاستحقاق التي تُنشأ فيها الإعانة (0 = في تاريخ الاستحقاق ذاته).',
        'title_template' => 'قالب العنوان (اختياري)',
        'title_template_hint' => 'يُعنون كل إعانة مولّدة. المتغيرات: {program} و{beneficiary} و{month} و{year} و{n}.',
        'title_preview' => 'معاينة:',
        'title_use_default' => 'استخدم القالب الافتراضي',
        'active' => 'الجدولة مفعّلة',
        'active_hint' => 'أوقفها لتعليق التكرار دون حذفه.',
        'frequency' => [
            'monthly' => 'شهري',
            'quarterly' => 'ربع سنوي',
            'semi_annual' => 'نصف سنوي',
            'yearly' => 'سنوي',
            'custom_months' => 'مخصص (بالأشهر)',
        ],
    ],

    'messages' => [
        'submitted' => 'تم تقديم الإعانة بنجاح للموافقة',
        'saved' => 'تم حفظ الإعانة بنجاح',
        'deleted' => 'تم حذف الإعانة بنجاح',
        'cancelled' => 'تم إلغاء الإعانة بنجاح',
        'bulk_created' => 'تم إنشاء :count إعانة بنجاح، إعانة مستقلة لكل مستفيد',
        'bulk_submitted' => 'تم تقديم :count إعانة بنجاح للموافقة',
    ],

    'programs' => [
        'messages' => [
            'saved' => 'تم حفظ برنامج الإعانة بنجاح',
            'cannot_delete_in_use' => 'لا يمكن حذف برنامج إعانة قيد الاستخدام',
            'deleted' => 'تم حذف برنامج الإعانة بنجاح',
        ],
    ],
];

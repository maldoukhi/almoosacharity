...
<header class="sticky top-0 z-20 flex h-16 shrink-0 items-center gap-3 border-b border-gray-100 bg-white/90 px-4 backdrop-blur sm:px-6 lg:px-8 dark:border-white/10 dark:bg-primary-950/90">
    ...
    <div class="flex shrink-0 items-center gap-2">
        @auth
            {{-- Global search trigger: the palette itself lives at the layout
                 root (the header's backdrop-blur would trap a fixed overlay),
                 so this only fires the window event the palette listens for. --}}
            <button
                type="button"
                x-data

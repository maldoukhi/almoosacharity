    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex items-center gap-2">
            ...
            @can('beneficiaries.import')
                <x-ui.button href="{{ route('admin.beneficiaries.import') }}" variant="secondary">
                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75V16.5m-13.5-9L12 3m0 0 4.5 4.5M12 3v13.5" />
                    </svg>
                    {{ __('beneficiaries.import.button') }}
                </x-ui.button>
            @endcan
                                        ...
                                        @if ($trashed)
                                            @can('restore', $beneficiary)
                                                <x-ui.button
                                                    variant="ghost"
                                                    size="sm"
                                                    data-confirm="{{ __('beneficiaries.confirm_restore') }}"
                                                    x-on:click="uiConfirm($el.dataset.confirm, () => $wire.restore({{ $beneficiary->id }}))"
                                                >
                                                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
                                        ...
                                        @else
                                            ...
                                            @can('delete', $beneficiary)
                                                <x-ui.button
                                                    variant="danger"
                                                    size="sm"
                                                    data-confirm="{{ __('beneficiaries.confirm_delete') }}"
                                                    x-on:click="uiConfirm($el.dataset.confirm, () => $wire.delete({{ $beneficiary->id }}), { danger: true })"
                                                >
                                                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">

...
<div class="p-6">
    <div class="mb-5 flex items-start gap-3">
        ...
        <div>
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white">
                {{ __('beneficiaries.flow.decision_modal.title', ['action' => $this->actionLabel]) }}
            </h2>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                {{ __('beneficiaries.flow.decision_modal.subtitle') }}
            </p>
        </div>
    </div>
    ...
    <dl class="divide-y divide-gray-100 rounded-(--radius-brand) border border-gray-100 dark:divide-white/10 dark:border-white/10">
        <div class="flex items-center justify-between gap-3 px-4 py-2.5">
            <dt class="text-sm text-gray-500 dark:text-gray-400">{{ __('beneficiaries.field_full_name') }}</dt>
            <dd class="text-sm font-medium text-gray-900 dark:text-white">{{ $beneficiary->full_name }}</dd>
        </div>
        <div class="flex items-center justify-between gap-3 px-4 py-2.5">
            <dt class="text-sm text-gray-500 dark:text-gray-400">{{ __('beneficiaries.flow.field_current_stage') }}</dt>
            <dd class="text-sm font-medium text-gray-900 dark:text-white">{{ $beneficiary->currentStage?->name ?? '—' }}</dd>
        </div>
    </dl>

    <form wire:submit="confirm" class="mt-5 space-y-5">
        <div>
            <label for="beneficiaryDecisionNote" class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-200">
                {{ __('beneficiaries.flow.field_decision_note') }}
                @if ($this->requiresNote)
                    <span class="text-status-rejected">*</span>
                @endif
            </label>
            <textarea
                id="beneficiaryDecisionNote"
                wire:model="note"
                rows="3"
        ...
        <div class="rounded-(--radius-brand) border {{ $tone['box'] }} p-4">
            <h3 class="text-sm font-semibold text-gray-900 dark:text-white">{{ __('beneficiaries.flow.decision_modal.effect_title') }}</h3>

            @if ($action === \App\Enums\ApprovalAction::Approve)
                @if ($this->isFinalApprove)
                    <p class="mt-2 text-sm leading-6 text-gray-700 dark:text-gray-200">{{ __('beneficiaries.flow.decision_modal.approve_final') }}</p>
                @else
                    <p class="mt-2 text-sm leading-6 text-gray-700 dark:text-gray-200">
                        {{ __('beneficiaries.flow.decision_modal.approve_next', ['stage' => $this->nextStage->name, 'role' => $this->nextReviewerRoleLabel]) }}
                    </p>
                    @if ($this->nextRecipientCount > 0)
                        <p class="mt-1 text-sm leading-6 text-gray-700 dark:text-gray-200">
                            {{ trans_choice('beneficiaries.flow.decision_modal.notify_recipients', $this->nextRecipientCount, ['count' => $this->nextRecipientCount]) }}
                        </p>
                    @endif
                @endif
            @elseif ($action === \App\Enums\ApprovalAction::Reject)
                <p class="mt-2 text-sm leading-6 text-gray-700 dark:text-gray-200">{{ __('beneficiaries.flow.decision_modal.reject_effect') }}</p>
            @else
                <p class="mt-2 text-sm leading-6 text-gray-700 dark:text-gray-200">{{ __('beneficiaries.flow.decision_modal.return_effect') }}</p>
            @endif
        </div>

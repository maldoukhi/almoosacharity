<div class="space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        @can('manage', \App\Models\AidProgram::class)
            <x-ui.button href="{{ route('admin.settings.aid-programs.create') }}" variant="primary">
    ...
    <div class="relative">
        ...
        <div wire:loading.remove wire:target="search">
            @if ($this->programs->isEmpty())
                <x-ui.empty-state :title="__('aid_programs.empty_title')" :description="__('aid_programs.empty_description')">
                    <x-slot:icon>
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-white/10">
                        @foreach ($this->programs as $program)
                            <tr class="transition duration-150 hover:bg-gray-50 dark:hover:bg-white/5">
                                <x-ui.table.td class="font-medium text-gray-900 dark:text-white">{{ $program->name }}</x-ui.table.td>
                                <x-ui.table.td>{{ $program->type->label() }}</x-ui.table.td>
                                <x-ui.table.td>
                                    {{ $program->approvalFlow?->name ?? __('aid_programs.default_flow') }}
                                </x-ui.table.td>
                                <x-ui.table.td align="end" class="tabular-nums">{{ $program->aids_count ?? $program->aids->count() }}</x-ui.table.td>
                                <x-ui.table.td>
                                    <button
                                        type="button"
                                ...
                                <x-ui.table.td align="end">
                                    <div class="flex items-center justify-end gap-1">
                                        @can('manage', $program)
                                            <x-ui.button href="{{ route('admin.settings.aid-programs.edit', $program) }}" variant="ghost" size="sm">
                                                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">

    ...
    <x-ui.card>
        <div class="mb-4">
            <p class="text-sm text-gray-500 dark:text-gray-400">
                {{ trans_choice('surveys.results_count', $this->surveys->count(), ['count' => $this->surveys->count()]) }}
            </p>
        </div>
                        ...
                        <tr class="transition duration-150 hover:bg-gray-50 dark:hover:bg-white/5" wire:key="survey-{{ $survey->id }}">
                            ...
                            <x-ui.table.td>
                                @can('update', $survey)
                                    <button
                                        type="button"
                                        data-confirm="{{ $survey->is_active ? __('surveys.confirm_toggle_deactivate') : __('surveys.confirm_toggle_activate') }}"
                                        x-on:click="uiConfirm($el.dataset.confirm, () => $wire.toggleActive({{ $survey->id }}), { danger: {{ $survey->is_active ? 'true' : 'false' }} })"
                                ...
                                <div class="flex items-center justify-end gap-1">
                                    ...
                                    @can('delete', $survey)
                                        <x-ui.button
                                            variant="danger"
                                            size="sm"
                                            data-confirm="{{ __('surveys.confirm_delete') }}"
                                            x-on:click="uiConfirm($el.dataset.confirm, () => $wire.delete({{ $survey->id }}), { danger: true })"
                                        >
                                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">

...
- [x] **إصلاحات تجربة**: تاب "السكن والدخل" (كان مفتاح التاب `housing_income` في البليد و`housing-income` في allow-list الخاص بـ `setTab` → النقر يُرفض بصمت)؛ قائمة المستفيدين في الدفعة الجماعية (تصنيف كفلتر اختياري + بحث بالاسم/الهوية + أهلية = كل الحالات عدا موقوف/معطّل/مرفوض، لا Active فقط، و`updatedSelectedIds` يطبّع القيم لـ int حتى يظهر حقل المبلغ المخصص لكل صف)؛ `housing_type` بمُحوِّل مرن (`tryFrom`، لا يرمي أبدًا)؛ رابط باني مسار حالة المستفيد في الشريط الجانبي (`beneficiaries.flows.configure`).
- [x] **موجة العنوان/الدورية + المستفيد + الموافقات** (تأسيس مشترك ثم 6 وكلاء متوازيين على ملفات منفصلة، 452 اختبار أخضر + Pint + مراجعة أمنية نظيفة ثم دمج على `prod`):
  - **عنوان الإعانة** (`aids.title` + `Aid::displayTitle`) حقل اختياري يظهر في رأس الإعانة والقوائم ونافذة التأكيد؛ و**قالب عنوان دوري** (`recurring_aid_plans.title_template` + `RecurringAidPlan::renderTitle` بمتغيرات `{program}/{beneficiary}/{month}/{year}/{n}`) يُعنون كل إعانة مولّدة.
  - **تاريخ استحقاق منفصل** (`recurring_aid_plans.due_on`) عن تاريخ بدء الجدولة (`starts_on`)؛ `next_run_on` يُبذَر من `due_on ?? starts_on`، و`isDue` لا يُطلق قبل `starts_on`.
  - **إضافة يدوية للسلسلة** (`GenerateRecurringAids::cloneIntoSeries` مشتركة بين المولّد المجدول وزر يدوي — للحالات الخاصة، تُنشئ مسودة خارج الجدول بلا تحريك `next_run_on`).
  - **نافذة تأكيد الإعانة** موسّعة (عنوان/برنامج/غرض/عدد أصناف/ملاحظة دورية).
  - **تفاصيل الإعانة**: بطاقة "ضمن سلسلة دورية" + رابط للخطة + رقم الدفعة، والعنوان في الرأس.
  - **المستفيد**: تبويب "الإعانات" في الملف؛ إصلاح ظهور أزرار دورة الحياة (إيقاف=Active فقط، إعادة تفعيل=موقوف/معطّل فقط، تعطيل=غير معطّل) وفق الانتقالات الصحيحة؛ **إعادة دراسة الحالة** (انتقال Active/Rejected/Deactivated→UnderReview + `ReopenBeneficiaryStudy` بوابة `beneficiaries.review`)؛ **اختيار مسار المراجعة** على النموذج (قابل للتغيير قبل بدء المراجعة فقط؛ `SubmitBeneficiary` صار يحترم المسار المُسنَد)؛ ترجمة أوصاف سجل النشاط (`beneficiary.*` عبر `beneficiaries.activity.events`).
  - **صندوق الموافقات**: تبويب "الموافقات السابقة" (قرارات المستخدم نفسه `where user_id = auth`).
  - **صلاحية مستقلة** `aids.recurring.manage` تحكم شاشة الإعانات الدورية.
  - **الشريط الجانبي**: مجموعة "إدارة المستخدمين" مستقلة؛ زر "تقرير الرسائل" السريع في شاشة الرسالة الجماعية.
- [x] **توحيد شاشة إنشاء الإعانة + قالب/معاينة العنوان + فلتر تصنيفات متعدد**:
  - **شاشة إنشاء موحّدة**: `Aids/Form` صارت الشاشة الوحيدة للإنشاء الفردي والجماعي — دُمجت فيها ميزات الدفعة (اختيار بالتصنيف عبر رقائق + رفع إكسل بأرقام الهوية على قرص خاص + مبلغ مخصص لكل مستفيد) بجانب النوع والعنوان والدورية. الاختيار بالتصنيف/الإكسل يدمج المؤهّلين فقط (غير موقوف/معطّل/مرفوض)، و`persist()` يعيد فحص الأهلية على مسار الكتابة (منع حقن مستفيد غير مؤهّل عبر تلاعب Livewire). الدورية مع عدة مستفيدين = خطة لكل مستفيد. `/aids/batch` صار redirect إلى `/aids/create` (تبقى `BatchCreate` لاختباراتها). `BeneficiaryFactory` صار افتراضه حالة مؤهّلة (كان عشوائيًا يشمل المحظورة).
  - **قالب العنوان الافتراضي + معاينة حيّة** (`RecurringAidPlan::DEFAULT_TITLE_TEMPLATE` + `renderTitleTemplate` مشتركة) على نموذج الإعانة وشاشة الخطط الدورية.
  - **فلتر التصنيفات متعدد الاختيار** في الرسالة الجماعية (whereHas+whereIn، بلا تكرار المستفيد).
- [x] **معالج إنشاء الإعانة (Wizard)**: الإنشاء صار 4 خطوات (المستفيدون ← التفاصيل ← الدورية والمستندات ← المراجعة) بمؤشر مراحل قابل للنقر وتحقق لكل خطوة (`validateStep` يرمي `ValidationException` — النمط الصحيح في Livewire؛ `save()` يبقى الفعل القانوني للحفظ حتى لا تنكسر الاختبارات/الأزرار، وEnter عبر `submitForm` يتقدم بالخطوات)، وزرا "حفظ كمسودة" و"حفظ وتقديم" في خطوة المراجعة؛ التعديل بقي صفحة واحدة. قائمة مراجعة للمستفيدين المختارين (اسم+هوية+حذف لكل صف + مسح الكل) تظهر لكل الأنواع. `AidWizardTest` (5 اختبارات).
- [x] **موجة الاحترافية (11 بندًا، 7 وكلاء متوازيون + تأسيس النسخ الاحتياطي يدويًا؛ 494 اختبار أخضر + مراجعة أمنية)**:
  - **نسخ احتياطي ليلي** (spatie/laravel-backup): مستندات `storage/app` + ملف SQLite على قرص `backups` مستقل (المصادر لا تشمل `.env`)، `backup:clean` 01:30 + `backup:run` 02:00، بريد عند الفشل فقط (`BACKUP_NOTIFICATION_EMAIL`)، تشفير عند ضبط `BACKUP_ARCHIVE_PASSWORD`، وdumps قواعد البيانات عبر `BACKUP_DATABASES=mysql` على الاستضافة (لا أدوات dump في حاوية التطوير).
  - **صفحة صحة النظام** `/admin/settings/system-health` (خلف `settings.view`): قائمة الانتظار/الفاشلة، فشل الرسائل 24س، آخر توليد دوري + الخطط المتأخرة (نبض المجدول)، آخر نسخة احتياطية وعمرها، مساحة القرص — كل بطاقة defensively لا تسقط الصفحة أبدًا.
  - **بطاقات مؤشرات تشغيلية** أعلى اللوحة: موافقات متأخرة >7 أيام، تأكيدات استلام منتهية بلا رد، دورية مستحقة خلال أسبوع، مستفيدون عالقون في المراجعة — بألوان حالة وروابط، خلف صلاحياتها.
  - **السجل الزمني الموحّد للإعانة**: بطاقة القرارات صارت خطًا زمنيًا يدمج الإنشاء/التقديم/القرارات/دورة الصرف/رابط التأكيد وردّ المستفيد؛ و**سند PDF** (`downloadReceipt` خلف gate view، هوية مقنّعة، بلا آيبان، نمط dompdf/IBM Plex نفسه) من حالة معتمدة فصاعدًا.
  - **SLA لمراحل الموافقة**: `approval_flow_stages.max_days` اختياري في الباني + أمر يومي `aids:escalate-overdue` (الزمن بالمرحلة = آخر قرار وإلا التقديم؛ تصعيد واحد لكل دخول مرحلة عبر `aids.approval_escalated_at` يعاد تسليحه عند تغيّر المرحلة) يُخطر `eligibleUsers` عبر الجرس، وشارة "متأخرة منذ n يوم" في صندوق الموافقات.
  - **تقرير الاستبيانات ببيانات فعلية**: تجميعات لكل سؤال (تقييم: متوسط/توزيع نجوم؛ اختيار: عدّ ونسب؛ نعم/لا؛ عينات نصية) بفلاتر استبيان/برنامج/فترة + Excel/PDF بنمط التقارير القائم؛ أُضيف `HasFactory` لـ`SurveyResponse`/`SurveyAnswer` مع factories.
  - **قفل السنة المالية**: إعداد `fiscal_locked_until_year` (شاشة `/admin/settings/fiscal-lock`) يمنع تعديل/إلغاء/حذف إعانات السنوات المقفلة — مفروض في `AidPolicy` (كل مسارات الواجهة ترثه) وفي `UpdateAid`/`CancelAid` عبر `App\Support\FiscalLock::assertMutable`.
  - **قالب استيراد رسمي** (تنزيل .xlsx بعناوين الأعمدة القانونية + صف مثال) و**تعرّف تلقائي** على القالب عند الرفع يملأ الربط مباشرة (`BeneficiaryImportTemplateExport::headers()` مصدر الحقيقة).
  - **بحث عام Ctrl+K** (`App\Livewire\GlobalSearch` في الشريط العلوي): مستفيدون (اسم/هوية/جوال) وإعانات (مرجع/عنوان)، كل مجموعة خلف صلاحيتها على مسار الاستعلام، بلا `@teleport`.
  - **فلاتر قوائم دائمة**: كل فلاتر قائمتي الإعانات والمستفيدين `#[Url]` + زر "مسح الفلاتر" + عدّاد الفلاتر النشطة.
  - **سد تسريب اللوحة** (من المراجعة الأمنية): إحصاءات اللوحة والرسوم وقائمة مشاكل الاستلام (أسماء مستفيدين + مبالغ شهرية) كانت تُعرض لأي مستخدم مصادَق — صارت كل computed تُرجع فارغًا بلا الصلاحية المطابقة + `@can` في البليد + اختبار انحدار.

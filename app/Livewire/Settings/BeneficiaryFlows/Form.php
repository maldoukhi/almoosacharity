...
use App\Actions\BeneficiaryFlows\SaveBeneficiaryFlow;
...
/**
 * Beneficiary review flow create/edit form (the flow itself plus its ordered
 * stages) — the configurable builder that mirrors the aid approval-flow
 * builder's UX: a spine of stage cards, role + specific-assignee multiselect,
 * and per-stage allowed actions.
 */
class Form extends Component
{
    public ?BeneficiaryFlow $flow = null;
    ...
    public function mount(?BeneficiaryFlow $flow = null): void
    {
        $this->flow = $flow;

        Gate::authorize('manage', $this->flow ?? BeneficiaryFlow::class);
        ...
        if ($this->flow?->exists) {
            ...
            $this->stages = $this->flow->stages()
                ->orderBy('order')
                ->get()
                ->map(fn (BeneficiaryFlowStage $stage): array => [
                    'name' => $stage->name,
                    'order' => $stage->order,
    ...
    public function removeStage(int $index): void
    {
        if (count($this->stages) <= 1) {
            $this->dispatch('toast', type: 'error', message: __('beneficiaries.flow.messages.at_least_one_stage'));
    ...
    public function save(): void
    {
        ...
        foreach ($this->stages as $index => $stage) {
            if (($stage['role'] ?? '') === '' && empty($stage['assignee_user_ids'])) {
                $this->addError("stages.{$index}.role", __('beneficiaries.flow.errors.stage_assignee_required'));
        ...
        try {
            app(SaveBeneficiaryFlow::class)->handle($data, $this->flow);
        ...
        $this->dispatch('toast', type: 'success', message: __('beneficiaries.flow.messages.saved'));

        $this->redirectRoute('admin.settings.beneficiary-flows.index', navigate: true);
    }
    ...
    {
        return view('livewire.settings.beneficiary-flows.form');
    }
}
